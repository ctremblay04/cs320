...
SetAssocPrefetchNoWrite :: SetAssocPrefetchNoWrite(int setSize) : Cache(SIZE-setSize) {
...
SetAssocPrefetchNoWrite :: ~SetAssocPrefetchNoWrite() {
...
int SetAssocPrefetchNoWrite :: updateCache(unsigned long long addr, int inst) {
  int match = this->updateCacheHelper(addr, inst);
  if (not match){ this->updateCacheHelper(addr+(1<<LINE_BITS), inst); }
  return match;
}

int SetAssocPrefetchNoWrite :: updateCacheHelper(unsigned long long addr, int inst) {
